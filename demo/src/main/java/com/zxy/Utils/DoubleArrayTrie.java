...
public class DoubleArrayTrie {
        ...
        public boolean exists(String word) {
            ...
            if (cur_p < base.length && (base[cur_p] + charCode(END_CHAR) < check.length) &&
                    base[base[cur_p] + charCode(END_CHAR)] == 0 &&
                    check[base[cur_p] + charCode(END_CHAR)]==cur_p)
                return true;
            return false;
        }

        public String filter(String replace, String str){
            StringBuffer sb = new StringBuffer("");
            for(int i = 0; i < str.length();) {
                int pre = 1;
                for (int j = i; j < str.length();) {
                    char c = str.charAt(j);
                    int cur = base[pre] + charCode(c);
                    if (cur >= check.length || check[cur] != pre) {
                        sb.append(str.charAt(i));
                        i++;
                        break;
                    }else{
                        if(base[cur] >= 0){
                            if(check[base[cur]+charCode(END_CHAR)] == cur){
                                for(int m = i; m <= j; m++)
                                    sb.append(replace);
                                i = j + 1;
                                break;
                            ...
                            j++;
                            int pos = -base[cur];
                            while(tail[pos] != END_CHAR && j < str.length()){
                                if(str.charAt(j) == tail[pos]){
                                    j++;
                                    pos++;
                                }
                                else{
